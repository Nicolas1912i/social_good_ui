...
export async function uploadStatus(
  ...
  return await getUploadStatus(accessToken, activityId).then((response) =>
    response.json().then((data) => {
      ...
      switch (activityStatus) {
        ...
        case ActivityStatus.timed_out: {
          ...
          setAlertMessage(
            "The server process found an error while importing CSV data, please try again later",
          );
        ...
        case ActivityStatus.completed: {
          ...
          setAlertMessage(
            "Successfully imported CSV data into constant contact!",
          );
